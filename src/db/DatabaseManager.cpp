...
bool DatabaseManager::open() {
    ...
    if (rc) return false;
    executeQuery("PRAGMA foreign_keys = ON;"); return true;
}
...
bool DatabaseManager::executeQuery(const std::string& sql) {
    ...
    if (rc != SQLITE_OK) { sqlite3_free(zErrMsg); return false; } return true;
}
...
std::vector<User> DatabaseManager::getAllUsers() {
    ...
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) { while (sqlite3_step(stmt) == SQLITE_ROW) users.push_back(User{ SAFE_TEXT(0), SAFE_TEXT(1), SAFE_TEXT(2), "", sqlite3_column_int(stmt, 4) != 0, SAFE_TEXT(3), SAFE_TEXT(5), 0, "", "" }); } sqlite3_finalize(stmt); return users;
}
bool DatabaseManager::isSystemAdmin(std::string userId) {
    sqlite3_stmt* stmt; bool isAdmin = false; std::string sql = "SELECT IsSystemAdmin FROM Users WHERE ID = ?;";
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) { sqlite3_bind_text(stmt, 1, userId.c_str(), -1, SQLITE_TRANSIENT); if (sqlite3_step(stmt) == SQLITE_ROW) isAdmin = (sqlite3_column_int(stmt, 0) == 1); } sqlite3_finalize(stmt); return isAdmin;
}
...
std::optional<User> DatabaseManager::getUserByGoogleId(const std::string& googleId) {
    ...
    if (sqlite3_prepare_v2(db, "SELECT ID, Name, Email, Status, IsSystemAdmin, AvatarURL, SubscriptionLevel, SubscriptionExpiresAt, GoogleID FROM Users WHERE GoogleID = ?;", -1, &stmt, nullptr) == SQLITE_OK) { sqlite3_bind_text(stmt, 1, googleId.c_str(), -1, SQLITE_STATIC); if (sqlite3_step(stmt) == SQLITE_ROW) user = User{ SAFE_TEXT(0), SAFE_TEXT(1), SAFE_TEXT(2), "", sqlite3_column_int(stmt, 4) != 0, SAFE_TEXT(3), SAFE_TEXT(5), sqlite3_column_int(stmt, 6), SAFE_TEXT(7), SAFE_TEXT(8) }; } sqlite3_finalize(stmt); return user;
}
bool DatabaseManager::createUser(const std::string& name, const std::string& email, const std::string& rawPassword, bool isAdmin) {
    ...
    sqlite3_bind_text(stmt, 1, id.c_str(), -1, SQLITE_STATIC); sqlite3_bind_text(stmt, 2, name.c_str(), -1, SQLITE_STATIC); sqlite3_bind_text(stmt, 3, email.c_str(), -1, SQLITE_STATIC); sqlite3_bind_text(stmt, 4, hash.c_str(), -1, SQLITE_STATIC); sqlite3_bind_int(stmt, 5, isAdmin ? 1 : 0); bool s = (sqlite3_step(stmt) == SQLITE_DONE); sqlite3_finalize(stmt); return s;
}
...
std::optional<User> DatabaseManager::getUser(const std::string& email) {
    sqlite3_stmt* stmt; std::optional<User> user = std::nullopt;
    if (sqlite3_prepare_v2(db, "SELECT ID, Name, Email, Status, IsSystemAdmin, AvatarURL, SubscriptionLevel, SubscriptionExpiresAt, GoogleID FROM Users WHERE Email = ?;", -1, &stmt, nullptr) == SQLITE_OK) { sqlite3_bind_text(stmt, 1, email.c_str(), -1, SQLITE_STATIC); if (sqlite3_step(stmt) == SQLITE_ROW) user = User{ SAFE_TEXT(0), SAFE_TEXT(1), SAFE_TEXT(2), "", sqlite3_column_int(stmt, 4) != 0, SAFE_TEXT(3), SAFE_TEXT(5), sqlite3_column_int(stmt, 6), SAFE_TEXT(7), SAFE_TEXT(8) }; } sqlite3_finalize(stmt); return user;
}
std::optional<User> DatabaseManager::getUserById(std::string id) {
    sqlite3_stmt* stmt; std::optional<User> user = std::nullopt;
    if (sqlite3_prepare_v2(db, "SELECT ID, Name, Email, Status, IsSystemAdmin, AvatarURL, SubscriptionLevel, SubscriptionExpiresAt, GoogleID FROM Users WHERE ID = ?;", -1, &stmt, nullptr) == SQLITE_OK) { sqlite3_bind_text(stmt, 1, id.c_str(), -1, SQLITE_STATIC); if (sqlite3_step(stmt) == SQLITE_ROW) user = User{ SAFE_TEXT(0), SAFE_TEXT(1), SAFE_TEXT(2), "", sqlite3_column_int(stmt, 4) != 0, SAFE_TEXT(3), SAFE_TEXT(5), sqlite3_column_int(stmt, 6), SAFE_TEXT(7), SAFE_TEXT(8) }; } sqlite3_finalize(stmt); return user;
}
...
bool DatabaseManager::updateUserDetails(std::string userId, const std::string& name, const std::string& status) {
    ...
    sqlite3_bind_text(stmt, 1, name.c_str(), -1, SQLITE_STATIC); sqlite3_bind_text(stmt, 2, status.c_str(), -1, SQLITE_STATIC); sqlite3_bind_text(stmt, 3, userId.c_str(), -1, SQLITE_STATIC); bool s = (sqlite3_step(stmt) == SQLITE_DONE); sqlite3_finalize(stmt); return s;
}
...
std::string DatabaseManager::authenticateUser(const std::string& email, const std::string& password) {
    ...
    if (sqlite3_prepare_v2(db, "SELECT ID, PasswordHash FROM Users WHERE Email = ?;", -1, &stmt, nullptr) == SQLITE_OK) { sqlite3_bind_text(stmt, 1, email.c_str(), -1, SQLITE_TRANSIENT); if (sqlite3_step(stmt) == SQLITE_ROW) { userId = SAFE_TEXT(0); dbPasswordHash = SAFE_TEXT(1); } sqlite3_finalize(stmt); }
    if (userId.empty() || !Security::verifyPassword(password, dbPasswordHash)) return ""; return userId;
}

// -------------------------------------------------------------
// YENİ EKLENEN SUNUCU/YETKİ/ENGELLEME FONKSİYONLARI 
// -------------------------------------------------------------
...
// -------------------------------------------------------------
// YENİ KANBAN FONKSİYONLARI (7 Parametreli Kart, Atama vb.)
// -------------------------------------------------------------
...
// -------------------------------------------------------------
// YENİ EKLENEN TEPKİLER VE THREAD (ALT YANIT)
// -------------------------------------------------------------
...
std::vector<Message> DatabaseManager::getThreadReplies(std::string messageId) {
    ...
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) { sqlite3_bind_text(stmt, 1, messageId.c_str(), -1, SQLITE_TRANSIENT); while (sqlite3_step(stmt) == SQLITE_ROW) { Message m; m.id = SAFE_TEXT(0); m.channel_id = messageId; m.sender_id = SAFE_TEXT(1); m.sender_name = SAFE_TEXT(2); m.sender_avatar = SAFE_TEXT(3); m.content = SAFE_TEXT(4); m.timestamp = SAFE_TEXT(5); replies.push_back(m); } } sqlite3_finalize(stmt); return replies;
}

// -------------------------------------------------------------
// ESKİ FONKSİYONLARIN DEVAMI (Değişmedi)
// -------------------------------------------------------------
...
bool DatabaseManager::createChannel(std::string serverId, std::string name, int type) {
    ...
    std::string id = Security::generateId(15); return executeQuery("INSERT INTO Channels (ID, ServerID, Name, Type) VALUES ('" + id + "', '" + serverId + "', '" + name + "', " + std::to_string(type) + ");");
}
...
std::vector<Channel> DatabaseManager::getServerChannels(std::string serverId) {
    ...
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) { while (sqlite3_step(stmt) == SQLITE_ROW) channels.push_back(Channel{ SAFE_TEXT(0), serverId, SAFE_TEXT(1), sqlite3_column_int(stmt, 2), false }); } sqlite3_finalize(stmt); return channels;
}
...
bool DatabaseManager::createKanbanCard(std::string listId, std::string title, std::string desc, int priority) { std::string id = Security::generateId(15); return executeQuery("INSERT INTO KanbanCards (ID, ListID, Title, Description, Priority, Position) VALUES ('" + id + "', '" + listId + "', '" + title + "', '" + desc + "', " + std::to_string(priority) + ", 0);"); }
...
bool DatabaseManager::logServerAction(const std::string& serverId, const std::string& action, const std::string& details) {
    ...
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) { sqlite3_bind_text(stmt, 1, serverId.c_str(), -1, SQLITE_TRANSIENT); sqlite3_bind_text(stmt, 2, action.c_str(), -1, SQLITE_TRANSIENT); sqlite3_bind_text(stmt, 3, details.c_str(), -1, SQLITE_TRANSIENT); bool success = (sqlite3_step(stmt) == SQLITE_DONE); sqlite3_finalize(stmt); return success; } return false;
}
std::vector<DatabaseManager::ServerLog> DatabaseManager::getServerLogs(const std::string& serverId) {
    ...
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) { sqlite3_bind_text(stmt, 1, serverId.c_str(), -1, SQLITE_TRANSIENT); while (sqlite3_step(stmt) == SQLITE_ROW) { logs.push_back({ "", serverId, "INFO", SAFE_TEXT(1), SAFE_TEXT(2), SAFE_TEXT(0) }); } } sqlite3_finalize(stmt); return logs;
}
...
bool DatabaseManager::sendServerInvite(std::string serverId, std::string inviterId, std::string inviteeId) {
    ...
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) { sqlite3_bind_text(stmt, 1, serverId.c_str(), -1, SQLITE_TRANSIENT); sqlite3_bind_text(stmt, 2, inviterId.c_str(), -1, SQLITE_TRANSIENT); sqlite3_bind_text(stmt, 3, inviteeId.c_str(), -1, SQLITE_TRANSIENT); bool success = (sqlite3_step(stmt) == SQLITE_DONE); sqlite3_finalize(stmt); return success; } return false;
}
...
bool DatabaseManager::markNotificationAsRead(int notifId) {
    ...
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) { sqlite3_bind_int(stmt, 1, notifId); bool success = (sqlite3_step(stmt) == SQLITE_DONE); sqlite3_finalize(stmt); return success; } return false;
}