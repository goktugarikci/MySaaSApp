...
bool DatabaseManager::open() {
    ...
    // İlişkisel bütünlük için foreign key desteğini aç
...
bool DatabaseManager::initTables() {
    std::string sql =
        ...
        "CREATE TABLE IF NOT EXISTS Payments (ID INTEGER PRIMARY KEY AUTOINCREMENT, UserID INTEGER, ProviderPaymentID TEXT, Amount REAL, Currency TEXT, Status TEXT DEFAULT 'pending', CreatedAt DATETIME DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(UserID) REFERENCES Users(ID));"
        "CREATE TABLE IF NOT EXISTS Reports (ID INTEGER PRIMARY KEY AUTOINCREMENT, ReporterID INTEGER, ContentID INTEGER, Type TEXT, Reason TEXT, Status TEXT DEFAULT 'OPEN', CreatedAt DATETIME DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(ReporterID) REFERENCES Users(ID));";

    return executeQuery(sql);
}

// =============================================================
// YÖNETİCİ VE İSTATİSTİKLER (HATA DÜZELTME: Scope kaldırıldı)
// =============================================================
...
bool DatabaseManager::isSystemAdmin(int userId) {
    std::string sql = "SELECT IsSystemAdmin FROM Users WHERE ID = " + std::to_string(userId) + ";";
    sqlite3_stmt* stmt;
    bool isAdmin = false;
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
...
bool DatabaseManager::deleteServer(int serverId) {
    return executeQuery("DELETE FROM Servers WHERE ID = " + std::to_string(serverId));
}
...
bool DatabaseManager::removeMemberFromServer(int serverId, int userId) {
    return executeQuery("DELETE FROM ServerMembers WHERE ServerID=" + std::to_string(serverId) + " AND UserID=" + std::to_string(userId));
}
...
bool DatabaseManager::createChannel(int serverId, std::string name, int type) {
    if (type == 3) {
        if (getServerKanbanCount(serverId) >= 1) return false;
    }
...
bool DatabaseManager::updateChannel(int channelId, const std::string& name) {
    return executeQuery("UPDATE Channels SET Name = '" + name + "' WHERE ID = " + std::to_string(channelId));
}

bool DatabaseManager::deleteChannel(int channelId) {
    return executeQuery("DELETE FROM Channels WHERE ID = " + std::to_string(channelId));
}
...
bool DatabaseManager::deleteMessage(int messageId) {
    return executeQuery("DELETE FROM Messages WHERE ID = " + std::to_string(messageId));
}
...
int DatabaseManager::getOrCreateDMChannel(int user1Id, int user2Id) {
    ...
    int channelId = -1;
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
        if (sqlite3_step(stmt) == SQLITE_ROW) channelId = sqlite3_column_int(stmt, 0);
    }
    sqlite3_finalize(stmt);
    if (channelId != -1) return channelId;
    sql = "INSERT INTO Channels (ServerID, Name, Type) VALUES (0, '" + dmName + "', 0);";
    if (executeQuery(sql)) return (int)sqlite3_last_insert_rowid(db);
    return -1;
}

// =============================================================
// KANBAN / TRELLO (HATA DÜZELTME: KanbanList kullanıldı)
// =============================================================
...
bool DatabaseManager::updateKanbanList(int listId, const std::string& title, int position) {
    return executeQuery("UPDATE KanbanLists SET Title='" + title + "', Position=" + std::to_string(position) + " WHERE ID=" + std::to_string(listId));
}
bool DatabaseManager::deleteKanbanList(int listId) {
    return executeQuery("DELETE FROM KanbanLists WHERE ID=" + std::to_string(listId));
}
...
bool DatabaseManager::updateKanbanCard(int cardId, std::string title, std::string desc, int priority) {
    return executeQuery("UPDATE KanbanCards SET Title='" + title + "', Description='" + desc + "', Priority=" + std::to_string(priority) + " WHERE ID=" + std::to_string(cardId));
}
bool DatabaseManager::deleteKanbanCard(int cardId) {
    return executeQuery("DELETE FROM KanbanCards WHERE ID=" + std::to_string(cardId));
}
bool DatabaseManager::moveCard(int cardId, int newListId, int newPosition) {
    return executeQuery("UPDATE KanbanCards SET ListID=" + std::to_string(newListId) + ", Position=" + std::to_string(newPosition) + " WHERE ID=" + std::to_string(cardId));
}
...
bool DatabaseManager::acceptFriendRequest(int requesterId, int myId) {
    return executeQuery("UPDATE Friends SET Status=1 WHERE RequesterID=" + std::to_string(requesterId) + " AND TargetID=" + std::to_string(myId));
}
...
std::vector<PaymentTransaction> DatabaseManager::getUserPayments(int userId) {
    std::vector<PaymentTransaction> payments;
    std::string sql = "SELECT ID, ProviderPaymentID, Amount, Currency, Status, CreatedAt FROM Payments WHERE UserID=" + std::to_string(userId);
    sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
        while (sqlite3_step(stmt) == SQLITE_ROW) {
...
// =============================================================
// RAPORLAMA (Hata Düzeltildi: resolveReport eklendi)
// =============================================================

bool DatabaseManager::createReport(int reporterId, int contentId, const std::string& type, const std::string& reason) {
    const char* sql = "INSERT INTO Reports (ReporterID, ContentID, Type, Reason) VALUES (?, ?, ?, ?);";
    sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) != SQLITE_OK) return false;
    sqlite3_bind_int(stmt, 1, reporterId);
...
std::vector<UserReport> DatabaseManager::getOpenReports() {
    ...
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            reports.push_back({ sqlite3_column_int(stmt, 0), sqlite3_column_int(stmt, 1), sqlite3_column_int(stmt, 2), SAFE_TEXT(3), SAFE_TEXT(4), SAFE_TEXT(5) });
        }
    }
    sqlite3_finalize(stmt);
    return reports;
}
bool DatabaseManager::resolveReport(int reportId) {
    return executeQuery("UPDATE Reports SET Status='RESOLVED' WHERE ID=" + std::to_string(reportId));
}