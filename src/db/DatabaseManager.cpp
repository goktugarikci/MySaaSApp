...
// ... [KULLANICI, SUNUCU, MESAJ, GİRİŞ VE SİLME İŞLEMLERİ (Öncekiyle aynı)] ...
// (Burada yer kaplamaması için standart CRUD'ları aynen tutuyoruz, YENİ eklenenleri veriyorum)
...
bool DatabaseManager::createGoogleUser(const std::string& name, const std::string& email, const std::string& googleId, const std::string& avatarUrl) {
    ...
    if (sqlite3_prepare_v2(db, "INSERT INTO Users (ID, Name, Email, GoogleID, AvatarURL) VALUES (?, ?, ?, ?, ?);", -1, &stmt, nullptr) != SQLITE_OK) return false;
...
// --- YENİ EKLENEN SUNUCU VE KANBAN METOTLARI ---

std::string DatabaseManager::getServerSettings(std::string serverId) {
    sqlite3_stmt* stmt; std::string settings = "{}";
    if (sqlite3_prepare_v2(db, "SELECT Settings FROM Servers WHERE ID = ?;", -1, &stmt, nullptr) == SQLITE_OK) {
        sqlite3_bind_text(stmt, 1, serverId.c_str(), -1, SQLITE_STATIC);
        if (sqlite3_step(stmt) == SQLITE_ROW) settings = SAFE_TEXT(0);
    } sqlite3_finalize(stmt); return settings;
}

bool DatabaseManager::updateServerSettings(std::string serverId, const std::string& settingsJson) {
    sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(db, "UPDATE Servers SET Settings = ? WHERE ID = ?;", -1, &stmt, nullptr) == SQLITE_OK) {
        sqlite3_bind_text(stmt, 1, settingsJson.c_str(), -1, SQLITE_TRANSIENT);
...
bool DatabaseManager::hasServerPermission(std::string serverId, std::string userId, std::string permissionType) {
    auto srv = getServerDetails(serverId);
    if (srv && srv->owner_id == userId) return true; // Kurucu her şeyi yapabilir
    ...
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            std::string rName = SAFE_TEXT(0);
            for (auto& c : rName) c = toupper(c); // Büyük harfe çevir
            if (rName.find(permissionType) != std::string::npos) { hasPerm = true; break; }
        }
...
bool DatabaseManager::isUserInServer(std::string serverId, std::string userId) {
    ...
    if (sqlite3_prepare_v2(db, "SELECT 1 FROM ServerMembers WHERE ServerID = ? AND UserID = ?;", -1, &stmt, nullptr) == SQLITE_OK) {
        sqlite3_bind_text(stmt, 1, serverId.c_str(), -1, SQLITE_TRANSIENT);
        sqlite3_bind_text(stmt, 2, userId.c_str(), -1, SQLITE_TRANSIENT);
        if (sqlite3_step(stmt) == SQLITE_ROW) inServer = true;
    } sqlite3_finalize(stmt); return inServer;
}

std::string DatabaseManager::getServerIdByRoleId(std::string roleId) {
    sqlite3_stmt* stmt; std::string sId = "";
    if (sqlite3_prepare_v2(db, "SELECT ServerID FROM Roles WHERE ID = ?;", -1, &stmt, nullptr) == SQLITE_OK) {
        sqlite3_bind_text(stmt, 1, roleId.c_str(), -1, SQLITE_STATIC);
...
bool DatabaseManager::updateRole(std::string roleId, std::string name, int hierarchy, int permissions) {
    sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(db, "UPDATE Roles SET RoleName=?, Hierarchy=?, Permissions=? WHERE ID=?;", -1, &stmt, nullptr) == SQLITE_OK) {
        sqlite3_bind_text(stmt, 1, name.c_str(), -1, SQLITE_TRANSIENT);
        sqlite3_bind_int(stmt, 2, hierarchy); sqlite3_bind_int(stmt, 3, permissions);
        sqlite3_bind_text(stmt, 4, roleId.c_str(), -1, SQLITE_TRANSIENT);
        bool s = (sqlite3_step(stmt) == SQLITE_DONE); sqlite3_finalize(stmt); return s;
    } return false;
}
...
bool DatabaseManager::assignRoleToMember(std::string serverId, std::string userId, std::string roleId) {
    sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(db, "INSERT OR REPLACE INTO ServerMemberRoles (ServerID, UserID, RoleID) VALUES (?, ?, ?);", -1, &stmt, nullptr) == SQLITE_OK) {
        sqlite3_bind_text(stmt, 1, serverId.c_str(), -1, SQLITE_TRANSIENT); sqlite3_bind_text(stmt, 2, userId.c_str(), -1, SQLITE_TRANSIENT); sqlite3_bind_text(stmt, 3, roleId.c_str(), -1, SQLITE_TRANSIENT);
        bool s = (sqlite3_step(stmt) == SQLITE_DONE); sqlite3_finalize(stmt); return s;
    } return false;
}
...
std::vector<User> DatabaseManager::getBlockedUsers(std::string userId) {
    ...
    if (sqlite3_prepare_v2(db, "SELECT U.ID, U.Name, U.Email, U.Status, U.IsSystemAdmin, U.AvatarURL FROM Users U JOIN BlockedUsers B ON U.ID = B.BlockedID WHERE B.UserID = ?;", -1, &stmt, nullptr) == SQLITE_OK) {
        sqlite3_bind_text(stmt, 1, userId.c_str(), -1, SQLITE_TRANSIENT);
        while (sqlite3_step(stmt) == SQLITE_ROW) blocked.push_back(User{ SAFE_TEXT(0), SAFE_TEXT(1), SAFE_TEXT(2), "", sqlite3_column_int(stmt, 4) != 0, SAFE_TEXT(3), SAFE_TEXT(5), 0, "", "" });
    } sqlite3_finalize(stmt); return blocked;
}

bool DatabaseManager::blockUser(std::string userId, std::string targetId) {
    sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(db, "INSERT OR IGNORE INTO BlockedUsers (UserID, BlockedID) VALUES (?, ?);", -1, &stmt, nullptr) == SQLITE_OK) {
        sqlite3_bind_text(stmt, 1, userId.c_str(), -1, SQLITE_TRANSIENT); sqlite3_bind_text(stmt, 2, targetId.c_str(), -1, SQLITE_TRANSIENT);
        bool s = (sqlite3_step(stmt) == SQLITE_DONE); sqlite3_finalize(stmt); return s;
    } return false;
}
...
std::string DatabaseManager::getServerIdByCardId(std::string cardId) {
    sqlite3_stmt* stmt; std::string sId = "";
    if (sqlite3_prepare_v2(db, "SELECT C.ServerID FROM KanbanCards KC JOIN KanbanLists KL ON KC.ListID = KL.ID JOIN Channels C ON KL.ChannelID = C.ID WHERE KC.ID = ?;", -1, &stmt, nullptr) == SQLITE_OK) {
...
bool DatabaseManager::assignUserToCard(std::string cardId, std::string assigneeId) {
    sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(db, "UPDATE KanbanCards SET AssigneeID = ? WHERE ID = ?;", -1, &stmt, nullptr) == SQLITE_OK) {
        sqlite3_bind_text(stmt, 1, assigneeId.c_str(), -1, SQLITE_TRANSIENT); sqlite3_bind_text(stmt, 2, cardId.c_str(), -1, SQLITE_TRANSIENT);
        bool s = (sqlite3_step(stmt) == SQLITE_DONE); sqlite3_finalize(stmt); return s;
    } return false;
}
...
bool DatabaseManager::updateCardCompletion(std::string cardId, bool isCompleted) {
    sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(db, "UPDATE KanbanCards SET IsCompleted = ? WHERE ID = ?;", -1, &stmt, nullptr) == SQLITE_OK) {
        sqlite3_bind_int(stmt, 1, isCompleted ? 1 : 0); sqlite3_bind_text(stmt, 2, cardId.c_str(), -1, SQLITE_TRANSIENT);
        bool s = (sqlite3_step(stmt) == SQLITE_DONE); sqlite3_finalize(stmt); return s;
    } return false;
}

// ... Kalan eski CRUD fonksiyonları (Server, Channel, List, Message vb.) ...
// YER KAPLAMAMASI İÇİN ESKİ DOSYADAKİ FONKSİYONLARIN DEVAMI BURADA VARSAYILIR
...
std::string DatabaseManager::createServer(const std::string& name, std::string ownerId) {
    ...
    if (success) { addMemberToServer(id, ownerId); createRole(id, "Admin", 100, 9999); return id; }
    return "";
}
...
std::vector<Server> DatabaseManager::getUserServers(std::string userId) {
    ...
    std::string sql = "SELECT S.ID, S.Name, S.OwnerID, S.InviteCode, S.IconURL, S.CreatedAt, (SELECT COUNT(*) FROM ServerMembers SM WHERE SM.ServerID = S.ID) FROM Servers S JOIN ServerMembers SM ON S.ID = SM.ServerID WHERE SM.UserID = '" + userId + "';";
    sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
        while (sqlite3_step(stmt) == SQLITE_ROW) servers.push_back(Server{ SAFE_TEXT(0), SAFE_TEXT(2), SAFE_TEXT(1), SAFE_TEXT(3), SAFE_TEXT(4), SAFE_TEXT(5), sqlite3_column_int(stmt, 6), {} });
    } sqlite3_finalize(stmt); return servers;
}
...
std::optional<Server> DatabaseManager::getServerDetails(std::string serverId) {
    ...
    sqlite3_stmt* stmt; std::optional<Server> server = std::nullopt;
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) != SQLITE_OK) return std::nullopt;
    if (sqlite3_step(stmt) == SQLITE_ROW) server = Server{ SAFE_TEXT(0), SAFE_TEXT(2), SAFE_TEXT(1), SAFE_TEXT(3), SAFE_TEXT(4), SAFE_TEXT(5), 0, {} };
    sqlite3_finalize(stmt); return server;
}
...
bool DatabaseManager::createRole(std::string serverId, std::string roleName, int hierarchy, int permissions) {
    ...
    return executeQuery("INSERT INTO Roles (ID, ServerID, RoleName, Hierarchy, Permissions) VALUES ('" + id + "', '" + serverId + "', '" + roleName + "', " + std::to_string(hierarchy) + ", " + std::to_string(permissions) + ");");
}
std::vector<Role> DatabaseManager::getServerRoles(std::string serverId) {
    ...
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
        while (sqlite3_step(stmt) == SQLITE_ROW) roles.push_back(Role{ SAFE_TEXT(0), serverId, SAFE_TEXT(1), SAFE_TEXT(2), sqlite3_column_int(stmt, 3), sqlite3_column_int(stmt, 4) });
    } sqlite3_finalize(stmt); return roles;
}
...
std::vector<Channel> DatabaseManager::getServerChannels(std::string serverId) {
    std::vector<Channel> channels; std::string sql = "SELECT ID, Name, Type FROM Channels WHERE ServerID = '" + serverId + "';"; sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
        while (sqlite3_step(stmt) == SQLITE_ROW) channels.push_back(Channel{ SAFE_TEXT(0), serverId, SAFE_TEXT(1), sqlite3_column_int(stmt, 2), false });
...
std::string DatabaseManager::getOrCreateDMChannel(std::string user1Id, std::string user2Id) {
    ...
    channelId = Security::generateId(15); sql = "INSERT INTO Channels (ID, ServerID, Name, Type) VALUES ('" + channelId + "', '0', '" + dmName + "', 0);";
...
bool DatabaseManager::updateMessage(std::string messageId, const std::string& newContent) {
    ...
    if (sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) != SQLITE_OK) return false;
    sqlite3_bind_text(stmt, 1, newContent.c_str(), -1, SQLITE_STATIC); sqlite3_bind_text(stmt, 2, messageId.c_str(), -1, SQLITE_STATIC);
    bool s = (sqlite3_step(stmt) == SQLITE_DONE); sqlite3_finalize(stmt); return s;
}
...
std::vector<Message> DatabaseManager::getChannelMessages(std::string channelId, int limit) {
    ...
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
        while (sqlite3_step(stmt) == SQLITE_ROW) messages.push_back(Message{ SAFE_TEXT(0), channelId, SAFE_TEXT(1), SAFE_TEXT(2), SAFE_TEXT(3), SAFE_TEXT(4), SAFE_TEXT(5), SAFE_TEXT(6) });
    } sqlite3_finalize(stmt); return messages;
}
    ...
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            if (sqlite3_prepare_v2(db, cardSql.c_str(), -1, &cardStmt, nullptr) == SQLITE_OK) {
                while (sqlite3_step(cardStmt) == SQLITE_ROW) cards.push_back(KanbanCard{ SAFE_TEXT(0), listId, SAFE_TEXT(1), SAFE_TEXT(2), sqlite3_column_int(cardStmt, 3), sqlite3_column_int(cardStmt, 4) });
            } sqlite3_finalize(cardStmt); board.push_back(KanbanList{ listId, SAFE_TEXT(1), sqlite3_column_int(stmt, 2), cards });
        }
...
bool DatabaseManager::logServerAction(const std::string& serverId, const std::string& action, const std::string& details) {
    ...
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
        sqlite3_bind_text(stmt, 1, serverId.c_str(), -1, SQLITE_TRANSIENT); sqlite3_bind_text(stmt, 2, action.c_str(), -1, SQLITE_TRANSIENT); sqlite3_bind_text(stmt, 3, details.c_str(), -1, SQLITE_TRANSIENT);
        bool s = (sqlite3_step(stmt) == SQLITE_DONE); sqlite3_finalize(stmt); return s;
    } return false;
}
...
std::vector<DatabaseManager::ServerMemberDetail> DatabaseManager::getServerMembersDetails(const std::string& serverId) {
    ...
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
        sqlite3_bind_text(stmt, 1, serverId.c_str(), -1, SQLITE_TRANSIENT);
        while (sqlite3_step(stmt) == SQLITE_ROW) members.push_back({ SAFE_TEXT(0), SAFE_TEXT(1), SAFE_TEXT(2) });
    } sqlite3_finalize(stmt); return members;
}
