...
// -------------------------------------------------------------
// KİMLİK DOĞRULAMA (AUTH) VE KULLANICI İŞLEMLERİ
// -------------------------------------------------------------
...
// -------------------------------------------------------------
// SİSTEM İSTATİSTİKLERİ VE LOGLAR
// -------------------------------------------------------------
...
std::vector<ServerLog> DatabaseManager::getSystemLogs(int limit) {
    ...
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
        while (sqlite3_step(stmt) == SQLITE_ROW) logs.push_back({ SAFE_TEXT(0), SAFE_TEXT(1), SAFE_TEXT(2), SAFE_TEXT(3), SAFE_TEXT(4), SAFE_TEXT(5) });
...
std::vector<Message> DatabaseManager::getThreadReplies(std::string messageId) {
    ...
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            Message m; m.id = SAFE_TEXT(0); m.channel_id = messageId; m.sender_id = SAFE_TEXT(1); m.sender_name = SAFE_TEXT(2); m.sender_avatar = SAFE_TEXT(3); m.content = SAFE_TEXT(4); m.timestamp = SAFE_TEXT(5); replies.push_back(m);
        }
...
std::vector<CardComment> DatabaseManager::getCardComments(std::string cardId) {
    ...
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) comments.push_back({ SAFE_TEXT(0), cardId, SAFE_TEXT(1), SAFE_TEXT(2), SAFE_TEXT(3), SAFE_TEXT(4) });
...
bool DatabaseManager::deleteCardComment(std::string commentId, std::string userId) { return executeQuery("DELETE FROM KanbanComments WHERE ID='" + commentId + "' AND UserID='" + userId + "'"); }
...
// -------------------------------------------------------------
// BİLDİRİM VE CRON JOB (NOTIFICATIONS)
// -------------------------------------------------------------
