...
#include <algorithm>
#include <cctype>
#include <fstream> // Excel/CSV disari aktarim icin EKLENDI
...
bool show_api_tester_window = false;
...
// =============================================================
// FULL ENDPOINT KÜTÜPHANESİ & POSTMAN DEĞİŞKENLERİ
// =============================================================
...
int api_last_status = 0;
std::atomic<bool> is_api_loading(false);
std::mutex api_response_mutex;
std::string thread_response_temp;
bool new_response_ready = false;

struct EndpointDef { const char* method; const char* url; const char* desc; const char* body; };
EndpointDef predefined_endpoints[] = {
    // --- 1. KİMLİK & AUTH ---
    {"POST", "/api/auth/login", "Giris Yap (Login)", "{\n  \"email\": \"admin@mysaas.com\",\n  \"password\": \"admin123\"\n}"},
    ...
    {"GET", "/api/users/search?q=Admin", "Kullanici Ara", ""},
    {"POST", "/api/user/status", "Manuel Durum Degistir", "{\n  \"status\": \"Rahatsiz Etmeyin\"\n}"},
    {"POST", "/api/user/ping", "Aktiflik Bildir (Keep-Alive)", ""},

    // --- 3. ARKADAŞLIK & DM ---
    {"GET", "/api/friends", "Arkadaslari Listele", ""},
    {"POST", "/api/friends/request", "Arkadas Ekle", "{\n  \"target_id\": \"HEDEF_ID\"\n}"},
    {"GET", "/api/friends/requests", "Gelen Istekleri Gor", ""},
    {"POST", "/api/users/dm", "Ozel Mesaj (DM) Baslat", "{\n  \"target_id\": \"HEDEF_ID\"\n}"},
    {"POST", "/api/friends/blocks", "Kullaniciyi Engelle", "{\n  \"target_id\": \"HEDEF_ID\"\n}"},

    // --- 4. SUNUCU & KANALLAR ---
    {"GET", "/api/servers", "Sunuculari Listele", ""},
    ...
    {"POST", "/api/servers/SERVER_ID/invites", "Davet Linki Olustur", ""},
    {"POST", "/api/servers/join/INVITE_CODE", "Sunucuya Katil", ""},

    // --- 5. MESAJLAŞMA ---
    {"GET", "/api/channels/CHANNEL_ID/messages", "Mesaj Gecmisini Cek", ""},
    {"POST", "/api/channels/CHANNEL_ID/messages", "Mesaj Gonder", "{\n  \"content\": \"Merhaba!\"\n}"},
    {"PUT", "/api/messages/MSG_ID", "Mesaji Duzenle", "{\n  \"content\": \"Duzenlenmis mesaj\"\n}"},
    {"POST", "/api/messages/MSG_ID/reactions", "Tepki Ekle (Emoji)", "{\n  \"reaction\": \"👍\"\n}"},

    // --- 6. KANBAN (PROJE YÖNETİMİ) ---
    {"GET", "/api/boards/CHANNEL_ID", "Kanban Panosu Getir", ""},
    {"POST", "/api/boards/CHANNEL_ID/lists", "Yeni Sütun Ekle", "{\n  \"title\": \"Yapilacaklar\"\n}"},
    {"PUT", "/api/lists/LIST_ID", "Sutun Adini Degistir", "{\n  \"title\": \"Guncel Sutun Adi\",\n  \"position\": 1\n}"},
    {"DELETE", "/api/lists/LIST_ID", "Sutunu Sil", ""},
    {"POST", "/api/lists/LIST_ID/cards", "Yeni Gorev Kart Ekle", "{\n  \"title\": \"Arayuz Fix\",\n  \"description\": \"Buton rengi degisecek\",\n  \"priority\": 2\n}"},
    {"PUT", "/api/cards/CARD_ID", "Gorevi Duzenle", "{\n  \"title\": \"Guncel Baslik\",\n  \"description\": \"Yeni detay\",\n  \"priority\": 3\n}"},
    {"DELETE", "/api/cards/CARD_ID", "Gorevi Sil", ""},
    {"PUT", "/api/cards/CARD_ID/move", "Gorevi Tasi (DragDrop)", "{\n  \"new_list_id\": \"HEDEF_SUTUN_ID\",\n  \"new_position\": 0\n}"},
    {"PUT", "/api/cards/CARD_ID/status", "Gorevi Tamamla", "{\n  \"is_completed\": true\n}"},
    {"POST", "/api/cards/CARD_ID/comments", "Karta Yorum Ekle", "{\n  \"content\": \"Bu islem acildir!\"\n}"},
    {"DELETE", "/api/comments/COMMENT_ID", "Yorumu Sil", ""},

    // --- 7. YÖNETİM, ŞİKAYET, UPLOAD & ÖDEME ---
    {"POST", "/api/upload", "Dosya Yukle (Multipart)", ""}, // Sadece referans, postman JSON atar, multipart desteklemez.
    {"GET", "/api/admin/reports", "Aktif Sikayetleri Listele", ""},
    {"POST", "/api/reports", "Sikayet Raporu Olustur", "{\n  \"content_id\": \"ID\",\n  \"type\": \"USER\",\n  \"reason\": \"Kufur/Hakaret\"\n}"},
    {"GET", "/api/admin/banlist", "Ban Listesini Getir", ""},
    {"POST", "/api/payments/checkout", "Yeni Odeme Baslat", "{\n  \"amount\": 99.99,\n  \"currency\": \"TRY\"\n}"}
};
...
void StopBackendServer() {
    ...
    if (pid != 0) {
        ...
        if (hProc) {
            ...
            AddConsoleLog("[SISTEM] Sunucu zorla kapatildi.");
    ...
    if (pid != 0) {
        ...
        if (hProc) {
            ...
            if (GetProcessTimes(hProc, &cT, &eT, &kT, &uT)) {
                ...
                ULARGE_INTEGER uct, unow; uct.LowPart = cT.dwLowDateTime; uct.HighPart = cT.dwHighDateTime;
                ...
                char buf[64]; sprintf_s(buf, "%02d:%02d:%02d", h, m, s); server_uptime_str = buf;
                if (!firstProcRun && pid == prevPid) {
                    ...
                    ULONGLONG totalSys = sysKDiff + sysUDiff; ULONGLONG totalProc = procKDiff + procUDiff;
        ...
        is_server_running = false; server_uptime_str = "00:00:00"; app_cpu_usage = 0.0f; app_ram_usage_mb = 0.0f; prevPid = 0; firstProcRun = true;
    }
    try {
...
// =============================================================
// ASENKRON API İSTEKLERİ (CANLI VERİ)
// =============================================================
void FetchUsersAsync() {
    if (isFetchingUsers) return; isFetchingUsers = true;
    std::thread([]() {
        ...
        if (r.status_code == 200) {
            try {
                ...
                if (!j.is_discarded() && j.is_array()) {
                    std::lock_guard<std::mutex> lock(userListMutex); userList.clear();
                    for (const auto& item : j) {
                        AdminUser u; u.id = item.value("id", "N/A"); u.name = item.value("name", "Unknown");
                        u.email = item.value("email", "N/A"); u.status = item.value("status", "Offline");
                        u.role = item.value("is_system_admin", 0) == 1 ? "System Admin" : "User";
                        int sub = item.value("subscription_level", 0); u.sub_level = (sub == 2) ? "Enterprise" : (sub == 1) ? "Pro" : "Normal";
...
void FetchServersAsync() {
    if (isFetchingServers) return; isFetchingServers = true;
    std::thread([]() {
        ...
        if (r.status_code == 200) {
            try {
                auto j = json::parse(r.text, nullptr, false);
                if (!j.is_discarded() && j.is_array()) {
                    std::lock_guard<std::mutex> lock(serverListMutex); serverList.clear();
                    for (const auto& item : j) {
                        AdminServer s; s.id = item.value("id", "N/A"); s.name = item.value("name", "Unknown");
                        s.owner_id = item.value("owner_id", "N/A"); s.member_count = item.value("member_count", 0);
...
void FetchBanListAsync() {
    if (isFetchingBans) return; isFetchingBans = true;
    std::thread([]() {
        ...
        if (r.status_code == 200) {
            try {
                auto j = json::parse(r.text, nullptr, false);
                if (!j.is_discarded() && j.is_array()) {
                    std::lock_guard<std::mutex> lock(banListMutex); banList.clear();
                    for (const auto& b : j) {
...
void FetchUserStatsAsync(std::string userId) {
    ...
    { std::lock_guard<std::mutex> lock(statsMutex); activeStatsServers.clear(); activeStatsFriends.clear(); activeStatsPayments.clear(); }
    std::thread([userId]() {
...
void FetchServerStatsAsync(std::string serverId) {
    if (isFetchingServerStats) return; isFetchingServerStats = true;
    { std::lock_guard<std::mutex> lock(statsMutex); activeServerMembersList.clear(); activeServerLogsList.clear(); }
    std::thread([serverId]() {
        cpr::Response r = cpr::Get(cpr::Url{ API_BASE_URL + "/api/admin/servers/" + serverId + "/detailed_members" }, cpr::Header{ {"Authorization", jwtToken} });
        if (r.status_code == 200) {
            try {
                auto j = json::parse(r.text, nullptr, false);
                if (!j.is_discarded() && j.contains("members") && j["members"].is_array()) {
                    std::lock_guard<std::mutex> lock(statsMutex);
                    for (const auto& m : j["members"]) {
                        activeServerMembersList.push_back({ m.value("user_id", ""), m.value("name", ""), m.value("status", "Offline") });
                    }
                }
            }
            catch (...) {}
...
void SendApiRequest() {
    if (is_api_loading) return; is_api_loading = true;
    strcpy_s(api_response_buffer, "Islem sunucuya iletiliyor..."); api_last_status = 0;

    std::string method = api_methods[api_method_idx];
    std::string endpoint = api_url_buffer;
    std::string body = api_body_buffer;
    if (endpoint.rfind("/", 0) != 0) endpoint = "/" + endpoint;
    ...
    std::thread([method, url, body, endpoint]() {
        ...
        if (method == "GET") r = cpr::Get(cpr::Url{ url }, headers);
        else if (method == "POST") r = cpr::Post(cpr::Url{ url }, headers, cpr::Body{ body });
        else if (method == "PUT") r = cpr::Put(cpr::Url{ url }, headers, cpr::Body{ body });
        else if (method == "DELETE") r = cpr::Delete(cpr::Url{ url }, headers);

        std::lock_guard<std::mutex> lock(api_response_mutex);
        api_last_status = r.status_code;
        try { auto j = nlohmann::json::parse(r.text); thread_response_temp = j.dump(4); }
        catch (...) { thread_response_temp = r.text.empty() ? "(Bos yanit veya HTML dondu)" : r.text; }

        if (r.status_code == 0) thread_response_temp = "Baglanti Hatasi: Sunucu kapali olabilir.";
        AddConsoleLog("[API YANIT] Status: " + std::to_string(api_last_status) + " -> " + endpoint);

        new_response_ready = true; is_api_loading = false;
        }).detach();
}

// =============================================================
// KONSOL VE KOMUT İŞLEMCİSİ
// =============================================================
std::vector<std::string> ParseCommand(const std::string& cmd) {
    std::istringstream iss(cmd); std::vector<std::string> tokens; std::string token;
    while (iss >> token) tokens.push_back(token); return tokens;
}

void ProcessConsoleCommand(const std::string& cmd) {
    if (cmd.empty()) return; AddConsoleLog("root@mysaas:~# " + cmd);
    auto args = ParseCommand(cmd);

    std::string action = args[0];
    std::transform(action.begin(), action.end(), action.begin(), ::tolower); // Büyük/Küçük harf duyarlılığını kaldır

    if (action == "clear") { std::lock_guard<std::mutex> lock(consoleLogMutex); consoleLog.clear(); }
    else if (action == "help") {
        AddConsoleLog("========== SISTEM KOMUTLARI ==========");
        AddConsoleLog(" SUNUCU  : start | stop | restart | uptime");
        AddConsoleLog(" BACKUP  : backup [isim] (Orn: backup veri1)");
        AddConsoleLog(" PANEL   : clear | refresh");
        AddConsoleLog(" YONETIM : ban <ID> | unban <ID> | banlist");
        AddConsoleLog(" GOZETIM : statsuser <ID> | statsserver <ID>");
    }
        ...
        try {
            if (!std::filesystem::exists("backups")) std::filesystem::create_directory("backups");
            std::string dest;
            if (args.size() > 1) {
                dest = "backups/" + args[1] + ".db";
            }
            else {
                auto t = std::time(nullptr); auto tm = *std::localtime(&t);
                const char* days[] = { "Paz", "Pzt", "Sal", "Car", "Per", "Cum", "Cmt" };
                char buf[256];
                sprintf_s(buf, "guncelleme_oncesi_%02d-%02d-%04d_%02d-%02d_%s.db", tm.tm_mday, tm.tm_mon + 1, tm.tm_year + 1900, tm.tm_hour, tm.tm_min, days[tm.tm_wday]);
                dest = "backups/" + std::string(buf);
            }
            if (std::filesystem::exists("mysaasapp.db")) {
                std::filesystem::copy_file("mysaasapp.db", dest, std::filesystem::copy_options::overwrite_existing);
                AddConsoleLog("[BASARILI] Veritabani yedeklendi -> " + dest);
            }
            else { AddConsoleLog("[HATA] 'mysaasapp.db' bulunamadi."); }
        }
        catch (const std::exception& e) { AddConsoleLog(std::string("[HATA] Yedekleme basarisiz: ") + e.what()); }
    }
    else if (action == "ban" && args.size() > 1) {
        std::string targetId = args[1]; AddConsoleLog("[BILGI] Yasaklaniyor: " + targetId);
        std::thread([targetId]() {
            cpr::Response r = cpr::Post(cpr::Url{ API_BASE_URL + "/api/admin/ban" }, cpr::Header{ {"Authorization", jwtToken}, {"Content-Type", "application/json"} }, cpr::Body{ "{\"user_id\":\"" + targetId + "\"}" });
            if (r.status_code == 200) { AddConsoleLog("[BASARILI] Yasaklandi."); FetchBanListAsync(); FetchUsersAsync(); }
            else AddConsoleLog("[HATA] Yasaklama basarisiz (" + std::to_string(r.status_code) + ")");
            }).detach();
    }
    else if (action == "unban" && args.size() > 1) {
        std::string targetId = args[1]; AddConsoleLog("[BILGI] Yasak kaldiriliyor: " + targetId);
        std::thread([targetId]() {
            cpr::Response r = cpr::Post(cpr::Url{ API_BASE_URL + "/api/admin/unban" }, cpr::Header{ {"Authorization", jwtToken}, {"Content-Type", "application/json"} }, cpr::Body{ "{\"user_id\":\"" + targetId + "\"}" });
            if (r.status_code == 200) { AddConsoleLog("[BASARILI] Yasak kaldirildi."); FetchBanListAsync(); FetchUsersAsync(); }
            else AddConsoleLog("[HATA] Islem basarisiz (" + std::to_string(r.status_code) + ")");
            }).detach();
    }
...
void DrawMainMenuBar() {
    if (ImGui::BeginMainMenuBar()) {
        if (ImGui::BeginMenu("Paneller")) {
            ...
            ImGui::MenuItem("Sistem Monitoru (Dashboard)", NULL, &show_dashboard);
            ImGui::MenuItem("Kullanici Yonetimi (CRM)", NULL, &show_user_management);
            ImGui::MenuItem("Sunucu Yonetimi (Workspace)", NULL, &show_server_management);
            ImGui::Separator();
            ImGui::MenuItem("Sistem Konsolu", NULL, &show_console_window);
            ImGui::MenuItem("API Test Araci (Postman)", NULL, &show_api_tester_window);
            ImGui::Separator();
            if (ImGui::MenuItem("Yasakli Kullanicilar (Banlist)")) { show_ban_list = true; FetchBanListAsync(); }
            ImGui::EndMenu();
        }
...
void DrawServerControlPanel() {
    ...
    ImGui::Begin(">> SUNUCU KONTROL MERKEZI <<", &show_server_control); ImGui::TextColored(ImVec4(1.0f, 0.8f, 0.0f, 1.0f), "Motor: MySaaSApp.exe"); ImGui::Separator(); ImGui::Text("Durum: "); ImGui::SameLine();
    ...
    if (ImGui::Button("Sunucuyu Baslat", ImVec2(180, 30))) { if (!is_server_running) StartBackendServer(); } ImGui::SameLine(); if (ImGui::Button("Sunucuyu Durdur", ImVec2(180, 30))) { if (is_server_running) StopBackendServer(); } ImGui::Spacing(); ImGui::TextColored(ImVec4(0.4f, 0.8f, 1.0f, 1.0f), "HTTP Trafik Loglari"); ImGui::Separator();
...
void DrawDashboard() {
    if (!show_dashboard) return; ImGui::SetNextWindowPos(ImVec2(10, 340), ImGuiCond_FirstUseEver); ImGui::SetNextWindowSize(ImVec2(450, 410), ImGuiCond_FirstUseEver);
    ImGui::Begin(">> SISTEM MONITORU <<", &show_dashboard); int t_users = 0, o_users = 0; { std::lock_guard<std::mutex> lock(userListMutex); t_users = userList.size(); for (const auto& u : userList) { if (u.status == "Online") o_users++; } }
    ImGui::TextColored(ImVec4(1.0f, 0.8f, 0.0f, 1.0f), "Ag Istatistikleri"); ImGui::Separator(); ImGui::Columns(3, "net_stats", false); ImGui::Text("Kayitli"); ImGui::TextColored(ImVec4(0.4f, 0.8f, 1.0f, 1.0f), "%d Kisi", t_users); ImGui::NextColumn(); ImGui::Text("Aktif"); ImGui::TextColored(ImVec4(0.2f, 1.0f, 0.2f, 1.0f), "%d Kisi", o_users); ImGui::NextColumn(); ImGui::Text("Sunucu"); ImGui::TextColored(ImVec4(1.0f, 0.4f, 0.4f, 1.0f), "%d Adet", (int)serverList.size()); ImGui::Columns(1); ImGui::Spacing();
    ImGui::TextColored(ImVec4(0.4f, 1.0f, 0.4f, 1.0f), "Kaynak Tuketimi"); ImGui::Separator(); char cpuOverlay[64]; sprintf_s(cpuOverlay, "CPU: %.2f%%", app_cpu_usage); ImGui::PlotLines("##AppCPU", app_cpu_graph, 90, time_offset, cpuOverlay, 0.0f, 100.0f, ImVec2(-1, 40)); char ramOverlay[64]; sprintf_s(ramOverlay, "RAM: %.2f MB", app_ram_usage_mb); ImGui::PlotLines("##AppRAM", app_ram_graph, 90, time_offset, ramOverlay, 0.0f, FLT_MAX, ImVec2(-1, 40)); ImGui::Spacing();
    ImGui::TextColored(ImVec4(0.4f, 0.8f, 1.0f, 1.0f), "Fiziksel Sunucu"); ImGui::Separator(); char sysRam[64]; sprintf_s(sysRam, "RAM: %.1f / %.1f GB", current_ram_used_gb, current_ram_total_gb); ImGui::ProgressBar(current_ram_percent, ImVec2(-1, 0), sysRam); ImGui::Spacing(); char diskOverlay[64]; sprintf_s(diskOverlay, "DB Boyutu: %.2f MB", db_size_mb); ImGui::ProgressBar(0.0f, ImVec2(-1, 0), diskOverlay); ImGui::End();
}

void DrawUserManagement() {
    if (!show_user_management) return; ImGui::SetNextWindowPos(ImVec2(470, 30), ImGuiCond_FirstUseEver); ImGui::SetNextWindowSize(ImVec2(880, 200), ImGuiCond_FirstUseEver);
    ImGui::Begin(">> KULLANICI YONETIMI (CRM) <<", &show_user_management); if (ImGui::Button("Yenile (Refresh)")) FetchUsersAsync(); if (isFetchingUsers) { ImGui::SameLine(); ImGui::TextColored(ImVec4(1, 1, 0, 1), " Guncelleniyor..."); } ImGui::Spacing();
    if (ImGui::BeginTable("UsersTable", 7, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_ScrollY)) {
        ImGui::TableSetupColumn("ID"); ImGui::TableSetupColumn("Ad Soyad"); ImGui::TableSetupColumn("E-Posta"); ImGui::TableSetupColumn("Durum"); ImGui::TableSetupColumn("Abonelik"); ImGui::TableSetupColumn("Yetki"); ImGui::TableSetupColumn("Islem"); ImGui::TableHeadersRow();
        std::lock_guard<std::mutex> lock(userListMutex);
        for (int i = 0; i < userList.size(); i++) {
            ImGui::TableNextRow(); ImGui::TableSetColumnIndex(0); ImGui::PushID(std::string("u_" + std::to_string(i)).c_str()); ImGui::Selectable(userList[i].id.c_str()); if (ImGui::BeginPopupContextItem()) { if (ImGui::Selectable("ID Kopyala")) ImGui::SetClipboardText(userList[i].id.c_str()); ImGui::EndPopup(); } ImGui::PopID();
            ImGui::TableSetColumnIndex(1); ImGui::Text("%s", userList[i].name.c_str()); ImGui::TableSetColumnIndex(2); ImGui::Text("%s", userList[i].email.c_str()); ImGui::TableSetColumnIndex(3);
            if (userList[i].status == "Online") ImGui::TextColored(ImVec4(0.2f, 1.0f, 0.2f, 1.0f), "Online"); else if (userList[i].status == "Banned") ImGui::TextColored(ImVec4(1.0f, 0.2f, 0.2f, 1.0f), "Banned"); else ImGui::TextColored(ImVec4(0.6f, 0.6f, 0.6f, 1.0f), "Offline");
            ImGui::TableSetColumnIndex(4); ImGui::Text("%s", userList[i].sub_level.c_str()); ImGui::TableSetColumnIndex(5); ImGui::Text("%s", userList[i].role.c_str()); ImGui::TableSetColumnIndex(6);
            std::string btn = "Detay##U" + std::to_string(i); if (ImGui::Button(btn.c_str())) ProcessConsoleCommand("statsuser " + userList[i].id); ImGui::SameLine(); std::string btnBan = "Ban##U" + std::to_string(i); if (ImGui::Button(btnBan.c_str())) ProcessConsoleCommand("ban " + userList[i].id);
        } ImGui::EndTable();
    } ImGui::End();
}

void DrawServerManagement() {
    if (!show_server_management) return; ImGui::SetNextWindowPos(ImVec2(470, 240), ImGuiCond_FirstUseEver); ImGui::SetNextWindowSize(ImVec2(880, 150), ImGuiCond_FirstUseEver);
    ImGui::Begin(">> SUNUCU (WORKSPACE) YONETIMI <<", &show_server_management); if (ImGui::Button("Yenile (Refresh)")) FetchServersAsync(); if (isFetchingServers) { ImGui::SameLine(); ImGui::TextColored(ImVec4(1, 1, 0, 1), " Guncelleniyor..."); } ImGui::Spacing();
    if (ImGui::BeginTable("ServersTable", 5, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_ScrollY)) {
        ImGui::TableSetupColumn("ID"); ImGui::TableSetupColumn("Sunucu Adi"); ImGui::TableSetupColumn("Kurucu ID"); ImGui::TableSetupColumn("Uye"); ImGui::TableSetupColumn("Islem"); ImGui::TableHeadersRow();
        std::lock_guard<std::mutex> lock(serverListMutex);
        for (int i = 0; i < serverList.size(); i++) {
            ImGui::TableNextRow(); ImGui::TableSetColumnIndex(0); ImGui::PushID(std::string("s_" + std::to_string(i)).c_str()); ImGui::Selectable(serverList[i].id.c_str()); if (ImGui::BeginPopupContextItem()) { if (ImGui::Selectable("ID Kopyala")) ImGui::SetClipboardText(serverList[i].id.c_str()); ImGui::EndPopup(); } ImGui::PopID();
            ImGui::TableSetColumnIndex(1); ImGui::Text("%s", serverList[i].name.c_str()); ImGui::TableSetColumnIndex(2); ImGui::Text("%s", serverList[i].owner_id.c_str()); ImGui::TableSetColumnIndex(3); ImGui::Text("%d", serverList[i].member_count); ImGui::TableSetColumnIndex(4);
            std::string btn = "Gozetim##S" + std::to_string(i); if (ImGui::Button(btn.c_str())) ProcessConsoleCommand("statsserver " + serverList[i].id);
        } ImGui::EndTable();
    } ImGui::End();
}

void DrawSystemConsoleWindow() {
    if (!show_console_window) return;
    ...
    if (ImGui::Begin(">> SISTEM KONSOLU <<", &show_console_window)) {
        ImGui::BeginChild("ConsoleRegion", ImVec2(0, -ImGui::GetFrameHeightWithSpacing()), ImGuiChildFlags_Border, ImGuiWindowFlags_HorizontalScrollbar); {
            ...
            for (const auto& log : consoleLog) {
                ...
                else if (log.find("[API ISTEGI]") != std::string::npos) ImGui::TextColored(ImVec4(1.0f, 0.6f, 0.0f, 1.0f), "%s", log.c_str());
                else if (log.find("[API YANIT]") != std::string::npos) ImGui::TextColored(ImVec4(0.4f, 0.8f, 1.0f, 1.0f), "%s", log.c_str());
                else if (log.find("[BASARILI]") != std::string::npos || log.find("[DOSYA]") != std::string::npos) ImGui::TextColored(ImVec4(0.2f, 1.0f, 0.2f, 1.0f), "%s", log.c_str());
                else ImGui::TextUnformatted(log.c_str());
            }
        }
        if (ImGui::GetScrollY() >= ImGui::GetScrollMaxY()) ImGui::SetScrollHereY(1.0f); ImGui::EndChild();
        ImGui::PushItemWidth(-60);
        if (ImGui::InputText("##Komut", consoleInput, IM_ARRAYSIZE(consoleInput), ImGuiInputTextFlags_EnterReturnsTrue)) {
            ProcessConsoleCommand(std::string(consoleInput)); strcpy_s(consoleInput, ""); ImGui::SetKeyboardFocusHere(-1);
        }
        ImGui::PopItemWidth(); ImGui::SameLine();
        if (ImGui::Button("Gonder")) { ProcessConsoleCommand(std::string(consoleInput)); strcpy_s(consoleInput, ""); }
    } ImGui::End();
}
...
void DrawApiTesterWindow() {
    ...
    if (ImGui::Begin(">> API TEST ARACI (POSTMAN) <<", &show_api_tester_window)) {
        ...
        ImGui::Columns(2, "PostmanColumns");
        ImGui::SetColumnWidth(0, 360);

        // --- SOL PANEL: HAZIR ENDPOINT LİSTESİ ---
        ImGui::TextColored(ImVec4(0.4f, 0.8f, 1.0f, 1.0f), "Tam Endpoint Kutuphanesi");
        ImGui::SameLine();

        // =========================================================================
        // MUHTEŞEM HTML DIŞA AKTARMA MOTORU BURADA BAŞLIYOR
        // =========================================================================
        if (ImGui::Button("HTML Dokuman Cikart", ImVec2(140, 20))) {
            std::ofstream file("api_documentation.html");
            if (file.is_open()) {
                // HTML ve CSS Başlıkları (Dark Mode Tema)
                std::string html_header = R"(
<!DOCTYPE html>
<html lang="tr">
<head>
    <meta charset="UTF-8">
    <title>MySaaS API Dokümantasyonu</title>
    <style>
        ...
        .auth-box { background-color: #161b22; border: 1px solid #30363d; border-radius: 8px; padding: 20px; margin-bottom: 20px; box-shadow: 0 4px 6px rgba(0,0,0,0.3); }
        .auth-box h3 { margin-top: 0; color: #3fb950; }
        .auth-box code { color: #ff7b72; background: #0d1117; padding: 4px 8px; border-radius: 4px; font-family: monospace; font-size: 14px;}
        table { width: 100%; border-collapse: collapse; background-color: #161b22; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.3); }
        th, td { border: 1px solid #30363d; padding: 12px 15px; text-align: left; vertical-align: top;}
        th { background-color: #21262d; color: #8b949e; font-weight: bold; font-size: 15px;}
        .method { padding: 5px 10px; border-radius: 4px; font-weight: bold; font-size: 12px; display: inline-block; width: 50px; text-align: center;}
        .m-GET { background-color: #238636; color: #ffffff; }
        .m-POST { background-color: #d29922; color: #ffffff; }
        .m-PUT { background-color: #1f6feb; color: #ffffff; }
        .m-DELETE { background-color: #da3633; color: #ffffff; }
        .url { font-family: monospace; font-size: 14px; color: #a5d6ff; font-weight: bold;}
        pre { margin: 0; color: #e6edf3; font-size: 13px; white-space: pre-wrap; word-wrap: break-word; background: #0d1117; padding: 10px; border-radius: 6px; border: 1px solid #30363d;}
    </style>
</head>
<body>
    <h1>🚀 MySaaS REST API Dokümantasyonu</h1>
    <div class="auth-box">
        <h3>🔐 Güvenlik ve Yetkilendirme (Bearer Token)</h3>
        <p>Frontend uygulaması, sunucuya istek atarken HTTP Request Header (Başlık) kısmına token eklemelidir:</p>
        <pre>Headers: {
  "Content-Type": "application/json",
  "Authorization": "Bearer <code>&lt;TOKEN_BURAYA_GELECEK&gt;</code>"
}</pre>
        <p><i>Not: Token almak için ilk olarak <b>POST /api/auth/login</b> adresine giriş isteği atılmalıdır.</i></p>
    </div>
    <table>
        <thead>
            <tr>
                <th style="width: 80px;">Metot</th>
                <th style="width: 250px;">Endpoint (URL)</th>
                <th style="width: 200px;">Açıklama</th>
                <th>JSON Gövdesi (Request Body)</th>
            </tr>
        </thead>
        <tbody>
)";
                ...
                // Dizideki her endpointi HTML tablosuna satır olarak ekle
                for (int i = 0; i < IM_ARRAYSIZE(predefined_endpoints); i++) {
                    std::string m = predefined_endpoints[i].method;
                    std::string mClass = "m-" + m; // CSS sınıfı için (Örn: m-GET)

                    std::string bodyText = predefined_endpoints[i].body;
                    if (bodyText.empty()) bodyText = "<span style='color:#8b949e'><i>JSON Body Gerekmez</i></span>";

                    file << "            <tr>\n";
                    file << "                <td><span class='method " << mClass << "'>" << m << "</span></td>\n";
                    file << "                <td class='url'>" << predefined_endpoints[i].url << "</td>\n";
                    file << "                <td>" << predefined_endpoints[i].desc << "</td>\n";
                    file << "                <td><pre>" << bodyText << "</pre></td>\n";
                    file << "            </tr>\n";
                }
                ...
                file << html_footer;
                file.close();
                AddConsoleLog("[BASARILI] Mukemmel API Dokumani 'api_documentation.html' olarak olusturuldu!");
            }
        }
        // =========================================================================
        // HTML DIŞA AKTARMA MOTORU BİTTİ
        // =========================================================================
        ...
        for (int i = 0; i < IM_ARRAYSIZE(predefined_endpoints); i++) {
            ...
            if (ImGui::IsItemHovered()) ImGui::SetTooltip("%s", predefined_endpoints[i].url);
        } ImGui::EndChild(); ImGui::NextColumn();

        // --- SAĞ PANEL: İSTEK OLUŞTURUCU VE YANIT ---
...
void DrawBanListModal() {
    if (!show_ban_list) return; ImGui::SetNextWindowPos(ImVec2(1366 / 2 - 350, 768 / 2 - 200), ImGuiCond_FirstUseEver); ImGui::SetNextWindowSize(ImVec2(700, 400), ImGuiCond_FirstUseEver);
    if (ImGui::Begin("Yasakli Kullanicilar (Ban Listesi)", &show_ban_list)) {
        if (ImGui::Button("Listeyi Yenile")) FetchBanListAsync(); ImGui::Spacing();
        if (ImGui::BeginTable("BanTable", 4, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_ScrollY)) { ImGui::TableSetupColumn("Kullanici ID"); ImGui::TableSetupColumn("Sebep"); ImGui::TableSetupColumn("Tarih"); ImGui::TableSetupColumn("Islem"); ImGui::TableHeadersRow(); std::lock_guard<std::mutex> lock(banListMutex); for (int i = 0; i < banList.size(); i++) { ImGui::TableNextRow(); ImGui::TableSetColumnIndex(0); ImGui::Text("%s", banList[i].user_id.c_str()); ImGui::TableSetColumnIndex(1); ImGui::Text("%s", banList[i].reason.c_str()); ImGui::TableSetColumnIndex(2); ImGui::Text("%s", banList[i].date.c_str()); ImGui::TableSetColumnIndex(3); std::string btn = "Kaldir##B" + std::to_string(i); if (ImGui::Button(btn.c_str())) ProcessConsoleCommand("unban " + banList[i].user_id); } ImGui::EndTable(); }
    } ImGui::End();
}

void DrawUserStatsModal() {
    if (!show_user_stats) return; ImGui::SetNextWindowPos(ImVec2(1366 / 2 - 400, 768 / 2 - 250), ImGuiCond_FirstUseEver); ImGui::SetNextWindowSize(ImVec2(800, 500), ImGuiCond_FirstUseEver); std::string title = "Kullanici Istihbarati: " + active_stats_id;
    if (ImGui::Begin(title.c_str(), &show_user_stats)) {
        if (isFetchingStats) ImGui::TextColored(ImVec4(1, 0.8f, 0, 1), "Veriler cekiliyor..."); else {
            std::lock_guard<std::mutex> lock(statsMutex); if (ImGui::BeginTabBar("UTabs")) {
                if (ImGui::BeginTabItem("Sunucular")) { if (activeStatsServers.empty()) ImGui::Text("Sunucu yok."); else { if (ImGui::BeginTable("UST", 3, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg)) { ImGui::TableSetupColumn("ID"); ImGui::TableSetupColumn("Ad"); ImGui::TableSetupColumn("Durum"); ImGui::TableHeadersRow(); for (const auto& s : activeStatsServers) { ImGui::TableNextRow(); ImGui::TableSetColumnIndex(0); ImGui::Text("%s", s.id.c_str()); ImGui::TableSetColumnIndex(1); ImGui::Text("%s", s.name.c_str()); ImGui::TableSetColumnIndex(2); if (s.owner_id == active_stats_id) ImGui::TextColored(ImVec4(0.2f, 1, 0.2f, 1), "Kurucu"); else ImGui::Text("Uye"); } ImGui::EndTable(); } } ImGui::EndTabItem(); }
                if (ImGui::BeginTabItem("DM/Arkadaslar")) { if (activeStatsFriends.empty()) ImGui::Text("Arkadas yok."); else { if (ImGui::BeginTable("UFT", 3, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg)) { ImGui::TableSetupColumn("ID"); ImGui::TableSetupColumn("Ad"); ImGui::TableSetupColumn("E-Posta"); ImGui::TableHeadersRow(); for (const auto& f : activeStatsFriends) { ImGui::TableNextRow(); ImGui::TableSetColumnIndex(0); ImGui::Text("%s", f.id.c_str()); ImGui::TableSetColumnIndex(1); ImGui::Text("%s", f.name.c_str()); ImGui::TableSetColumnIndex(2); ImGui::Text("%s", f.email.c_str()); } ImGui::EndTable(); } } ImGui::EndTabItem(); }
                ImGui::EndTabBar();
            }
        }
    } ImGui::End();
...
void DrawServerStatsModal() {
    if (!show_server_stats) return; ImGui::SetNextWindowPos(ImVec2(1366 / 2 - 400, 768 / 2 - 300), ImGuiCond_FirstUseEver); ImGui::SetNextWindowSize(ImVec2(800, 600), ImGuiCond_FirstUseEver); std::string title = "Sunucu Gozetimi: " + active_stats_id;
    if (ImGui::Begin(title.c_str(), &show_server_stats)) {
        if (isFetchingServerStats) ImGui::TextColored(ImVec4(1, 0.8f, 0, 1), "Cekiliyor..."); else {
            ImGui::Columns(2, "SCols"); ImGui::SetColumnWidth(0, 300); ImGui::TextColored(ImVec4(0.4f, 0.8f, 1, 1), "Uyeler"); ImGui::BeginChild("SMR", ImVec2(0, 0), true); { std::lock_guard<std::mutex> lock(statsMutex); if (activeServerMembersList.empty()) ImGui::Text("Uye yok."); else { if (ImGui::BeginTable("SMT", 2, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg)) { ImGui::TableSetupColumn("Ad"); ImGui::TableSetupColumn("Durum"); ImGui::TableHeadersRow(); for (const auto& m : activeServerMembersList) { ImGui::TableNextRow(); ImGui::TableSetColumnIndex(0); ImGui::Text("%s", m.name.c_str()); ImGui::TableSetColumnIndex(1); if (m.status == "Online") ImGui::TextColored(ImVec4(0.2f, 1, 0.2f, 1), "Online"); else ImGui::Text("Offline"); } ImGui::EndTable(); } } } ImGui::EndChild(); ImGui::NextColumn();
            ImGui::TextColored(ImVec4(0.4f, 0.8f, 1, 1), "Loglar"); ImGui::BeginChild("SLR", ImVec2(0, 0), true); { std::lock_guard<std::mutex> lock(statsMutex); if (activeServerLogsList.empty()) ImGui::Text("Kayıt yok."); else { if (ImGui::BeginTable("SLT", 3, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg)) { ImGui::TableSetupColumn("Tarih", ImGuiTableColumnFlags_WidthFixed, 130); ImGui::TableSetupColumn("Aksiyon", ImGuiTableColumnFlags_WidthFixed, 100); ImGui::TableSetupColumn("Detay"); ImGui::TableHeadersRow(); for (const auto& log : activeServerLogsList) { ImGui::TableNextRow(); ImGui::TableSetColumnIndex(0); ImGui::Text("%s", log.time.c_str()); ImGui::TableSetColumnIndex(1); ImGui::TextColored(ImVec4(1, 0.8f, 0, 1), "%s", log.action.c_str()); ImGui::TableSetColumnIndex(2); ImGui::TextWrapped("%s", log.details.c_str()); } ImGui::EndTable(); } } } ImGui::EndChild(); ImGui::Columns(1);
        }
    } ImGui::End();
}
...
int main() {
    ...
    if (!window) return -1; glfwMakeContextCurrent(window); glfwSwapInterval(1);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // Donanım Değerleri Senkronizasyonu (0.5 saniyede bir)
        if (current_time - last_update_time > 0.5) {
            UpdateHardwareMetrics();
            app_cpu_graph[time_offset] = app_cpu_usage; app_ram_graph[time_offset] = app_ram_usage_mb; db_size_graph[time_offset] = db_size_mb;
            time_offset = (time_offset + 1) % 90; last_update_time = current_time;
        }

        // Veri Tablosu Senkronizasyonu (3.0 saniyede bir)
        if (is_server_running && (current_time - last_sync_time > 3.0)) {
            FetchUsersAsync(); FetchServersAsync(); FetchBanListAsync(); last_sync_time = current_time;
        }
